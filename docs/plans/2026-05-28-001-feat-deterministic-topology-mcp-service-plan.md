...
- KTD5. **Agent MCP summary 与本地 full 数据通道分离:** Agent-facing MCP adapter 强制 summary，并拒绝 `full` responseMode；stage runner/project bridge 在本地进程内直接调用同一 topology domain 获取 full `IntermediateTopology`，full artifact/changeSet 只进入本地 project/client storage，不进入 Agent 上下文。
...
flowchart TB
  ...
  Canonical --> Client
  Project -.summary only.-> Agent[Agent runtime]
```
...
P0 明确使用两条数据通道：Agent MCP tool response 只承载 summary；本地 stage runner、fake agent 和 client wrapper 通过同一 topology domain package 获取 full `IntermediateTopology`，再交给 project bridge。任何 Agent-facing MCP 请求 `responseMode: "full"` 都应返回 `FORBIDDEN_RESPONSE_MODE`，而不是把 full topology 或 artifact 放进模型上下文。
...
- Agent-facing MCP handler 对 `responseMode: "full"` 返回 `FORBIDDEN_RESPONSE_MODE`；full result 只能由本地 wrapper/domain 调用路径获取。
- summary 模式不返回 full artifact、full port table 或 full changeSet。
...
- Agent MCP 工具只用于生成可展示摘要和结构化诊断；stage result 所需 full topology 由 stage runner/project bridge 在本地调用 topology domain 获取，不能从 Agent tool response 中解析 full payload。
...
- MCP call failure 被记录为脱敏 `call_failed` 摘要，不记录 full topology data。
...
- full topology、artifact、端口占用和 dryRun changeSet 只保存在本地 project/session storage；dryRun full 数据默认会话内保存，取消确认、关闭会话或删除项目时清理，诊断日志和 Agent 上下文不得保存 full 数据。
...
- Domain-level: `src/topology/*.test.ts` 覆盖 schema、templates、initialize、validate、artifacts、inspect、operations 和 limits。
...
- Origin requirements: `docs/brainstorms/2026-05-27-tsn-topology-mcp-requirements.md`
