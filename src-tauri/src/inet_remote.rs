...
impl RemoteRunner for SshRunner {
    fn run_bundle(&self, bundle: &InetBundle, cfg: &RemoteConfig) -> Result<RemoteRunOutcome, RemoteError> {
        ...
        if let Err(e) = run_with_timeout(make_cmd(&ssh, build_ssh_argv(cfg, &mkdir_cmd)), cfg.timeout) {
            cleanup_local();
            return Err(RemoteError::Unreachable(format!("连远端/建目录失败：{}", redact_remote(&e.to_string(), cfg))));
        }
        // 2) scp 传内容。
        let local_src = format!("{}/.", local_dir.display());
        if let Err(e) = run_with_timeout(make_cmd(&scp, build_scp_argv(cfg, &local_src, &remote_dir)), cfg.timeout) {
            let _ = run_with_timeout(make_cmd(&ssh, build_ssh_argv(cfg, &remote_cleanup_cmd(&remote_dir))), cfg.timeout);
            cleanup_local();
            return Err(RemoteError::Unreachable(format!("传输 bundle 失败：{}", redact_remote(&e.to_string(), cfg))));
        }
        // 3) 跑 inet。
        ...
        match run_res {
            Ok(output) => {
                ...
                Ok(RemoteRunOutcome {
                    ...
                    output_tail: tail(&redact_remote(&combined, cfg), 2000),
                })
            }
...
/// 假设远端输出量小（仿真日志几十行），piped buffer 不会阻塞。
