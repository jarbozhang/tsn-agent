...
- **`dual-plane-redundant`**：用户要 A/B 双平面、端系统双归属冗余时使用。典型场景：宇航双平面验收的**单跳**（端系统经一组 A/B 交换机双归属）与**双跳**（两组 A/B 交换机，平面内 line 骨干级联）。当前支持 `backbone=line` + `crossPlaneLinks=none` + 端系统主备双归属；参数字段名与合法域以 `describe_templates` 返回为准。
