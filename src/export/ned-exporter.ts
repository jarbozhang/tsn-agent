...
export function exportNed(project: CanonicalTsnProjectV0): string {
  ...
  return [
    ...
    imports,
    "",
    `network ${NED_CONTRACT.networkName}`,
    "{",
    "    parameters:",
    '        @display("bgb=980,460");',
    "    submodules:",
...
function renderSubmodule(node: TsnNode): string {
  const moduleType = node.type === "switch" ? "EthernetSwitch" : "StandardHost";
  return [
...
function renderConnection(projectLink: CanonicalTsnProjectV0["topology"]["links"][number]): string {
  ...
  return [
    `        ${safeNedId(projectLink.source.nodeId)}.ethg++ <--> EthernetLink { datarate = ${dataRate}; } <--> ${safeNedId(projectLink.target.nodeId)}.ethg++;`,
  ].join("");
}
