...
test("beginner request generates topology and export files", async ({ page }) => {
  ...
  await expect(page.getByRole("status").getByText("已导出 5 个文件：browser-preview")).toBeVisible();
  await page.getByRole("button", { name: "日志" }).click();
  await expect(page.getByRole("complementary", { name: "诊断日志" })).toBeVisible();
  await expect(page.getByLabel("当前会话诊断日志").getByText("用户提交需求")).toBeVisible();
  await expect(page.getByLabel("当前会话诊断日志").getByText("artifact bundle 已生成")).toBeVisible();
  await expect(page.getByLabel("当前会话诊断日志").getByText("项目文件已导出")).toBeVisible();
});
