...
test("beginner request moves through staged workflow and exports files", async ({ page }) => {
  ...
  await expect(page.getByLabel("导出文件列表").getByText("完成“模拟仿真”阶段后显示项目导出文件")).toBeVisible();
  ...
  await page.getByRole("tab", { name: "导出文件" }).click();
  await expect(page.getByLabel("导出文件列表").getByText("simulation/inet/tsnagent/generated/network.ned", { exact: true })).toBeVisible();
  await expect(page.getByLabel("导出文件列表").getByText("simulation/inet/omnetpp.ini", { exact: true })).toBeVisible();
  await expect(page.getByLabel("导出文件列表").getByText("simulation/inet/traffic.ini", { exact: true })).toBeVisible();
